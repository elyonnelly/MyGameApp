...
{
    public class AllowSpellsController : MonoBehaviour
    {
        ...
        private void FillTableOfSpells()
        {
        ...
        private GameObject CreateNewSpellSprite(string name, Vector3 position, string type)
        {
            var newSpell = Instantiate(Spell, position, Quaternion.identity);
            newSpell.SetActive(false);
            newSpell.name = name;
            newSpell.GetComponent<SpriteRenderer>().sprite = Resources.Load<Sprite>(type == "off" ? $"Sprites/Spells Icon/Offensive Spells/{name}" : $"Sprites/Spells Icon/Defensive Spells/{name}");
